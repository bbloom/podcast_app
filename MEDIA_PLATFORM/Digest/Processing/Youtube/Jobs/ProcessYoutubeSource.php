...
namespace MediaPlatform\Digest\Processing\Youtube\Jobs;
...
class ProcessYoutubeSource implements ShouldQueue
{
    ...
    public function handle(YoutubeContentProcessor $processor): void
    {
        ...
        if (! $listSource || ! $listSource->enabled || $listSource->suspended) {
            Log::info("ProcessYoutubeSource: list_source {$this->listSourceId} not found, disabled, or suspended. Skipping.");
            return;
        }

        Log::info("ProcessYoutubeSource: Processing list_source {$this->listSourceId}.");

        $stats = $processor->process($listSource);

        Log::info("ProcessYoutubeSource: Complete for list_source {$this->listSourceId}.", $stats);
    }

    public function failed(\Throwable $e): void
    {
        Log::error("ProcessYoutubeSource: All retries exhausted for list_source {$this->listSourceId}.", [
            'error' => $e->getMessage(),
        ]);
        ...
        if ($tracking->shouldSuspend()) {
            ...
            AdminAlert::raiseIfNew(
                tier: 2,
                category: 'youtube',
                title: "YouTube source auto-suspended (list_source {$this->listSourceId})",
