...
namespace MediaPlatform\Podcasts\Planning\EditScriptField\Controllers;
...
class EditScriptFieldController extends Controller
{
    /**
    ...
    public function show(PodcastEpisodePlanning $podcast_episode_planning): View|RedirectResponse
    {
        ...
        return view('media_platform.podcasts.planning.edit_script_field.edit', [
            'episode' => $podcast_episode_planning,
        ]);
    }

    /**
     * Save the script and return a JSON response.
    ...
    public function save(Request $request, PodcastEpisodePlanning $podcast_episode_planning): JsonResponse
    {
        ...
        $podcast_episode_planning->update(['script' => $request->input('script')]);

        return response()->json(['success' => true, 'message' => 'Script saved.']);
    }

    /**
     * Save the script and redirect to the episode show page.
