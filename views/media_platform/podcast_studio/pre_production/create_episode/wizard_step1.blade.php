<x-layouts.app title="Create Podcast Episode">

    {{-- Wizard header --}}
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-gray-800">Create Podcast Episode</h1>
        @include('media_platform.podcast_studio.pre_production.create_episode._step_dots', ['current' => 1])
    ...
    <div class="border border-purple-500 rounded-lg overflow-hidden">
        @foreach ($shows as $show)
            <div class="flex items-center gap-6 px-6 py-4 bg-white {{ ! $loop->last ? 'border-b border-purple-500' : '' }}">
                ...
                {{-- Select button --}}
                <div class="flex-shrink-0">
                    <form action="{{ route('pre_production_create_podcast_episode.step1.store') }}" method="POST">
                        @csrf
