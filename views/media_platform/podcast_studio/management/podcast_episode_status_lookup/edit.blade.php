<x-layouts.app title="Edit Episode Status">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold text-gray-800">Edit Episode Status</h1>
    </div>

    <form method="POST" action="{{ route('podcast_episode_status_lookup.update', $status) }}">
        @csrf
        @method('PUT')
            ...
            <div class="flex gap-3">
                <label class="flex-1 cursor-pointer">
                    <input type="radio" name="enabled" value="1"
                        {{ old('enabled', $status->enabled ? '1' : '0') === '1' ? 'checked' : '' }}
                        class="sr-only peer">
                ...
                <label class="flex-1 cursor-pointer">
                    <input type="radio" name="enabled" value="0"
                        {{ old('enabled', $status->enabled ? '1' : '0') === '0' ? 'checked' : '' }}
                        class="sr-only peer">
                    <div class="border border-gray-300 rounded-lg px-4 py-3 text-sm font-semibold text-center text-gray-700 peer-checked:border-purple-700 peer-checked:text-purple-700 peer-checked:bg-purple-50 hover:border-gray-400 transition">
                        Disabled
                    </div>
