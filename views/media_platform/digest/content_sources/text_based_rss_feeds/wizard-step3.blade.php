<x-layouts.app title="Add a Text Based RSS Feed">
    ...
    <form method="POST" action="{{ route('text_based_rss_feeds.create.step3.submit') }}"
          x-data="{
              listStates: {
        ...
        <div class="mb-8">
            <label class="block text-sm font-semibold text-gray-700 mb-3">
                Which lists should <span class="text-purple-700">{{ $feedTitle }}</span> be added to?
            </label>
                ...
                <div class="bg-amber-50 border border-amber-300 rounded-lg p-4">
                    <div class="flex gap-3">
                        ...
                        <div class="text-sm text-amber-800">
                            <p class="font-semibold mb-1">No lists yet</p>
                            <p>You need at least one list before you can add a feed. Create a list first, then come back to add this feed.</p>
                            <a href="{{ route('lists.create.step1') }}" class="inline-block mt-3 text-purple-700 font-semibold hover:underline">
                                Create a list →
                            </a>
                        </div>
                    </div>
        ...
        <div class="flex justify-between items-center">
            <a href="{{ route('text_based_rss_feeds.create.step2') }}"
