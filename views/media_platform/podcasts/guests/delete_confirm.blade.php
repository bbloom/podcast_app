<x-layouts.app title="Delete Podcast Guest">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold text-gray-800">Delete Podcast Guest</h1>
    </div>
    ...
    @php $episodeCount = $guest->episodes()->count(); @endphp

    @if ($episodeCount > 0)

        <div class="border border-red-200 bg-red-50 rounded-lg px-6 py-5 mb-8">
            <p class="text-sm font-semibold text-red-700 mb-1">This guest cannot be deleted.</p>
            <p class="text-sm text-red-600">
                <strong>{{ $guest->full_name }}</strong> is attached to
                {{ $episodeCount }} {{ Str::plural('episode', $episodeCount) }}.
                Please detach them from all episodes before deleting.
            </p>
        </div>
    ...
    @else
        ...
        <div class="border border-red-200 bg-red-50 rounded-lg px-6 py-5 mb-8">
            <p class="text-sm font-semibold text-red-700 mb-1">Are you sure you want to delete this guest?</p>
            <p class="text-sm text-red-600"><strong>{{ $guest->full_name }}</strong></p>
            <p class="mt-3 text-xs text-red-500">This action cannot be undone.</p>
        </div>

        <form method="POST" action="{{ route('podcast_guests.destroy', $guest) }}">
            @csrf
            @method('DELETE')
