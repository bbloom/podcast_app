...
- PHP Enums live in an `Enums/` subfolder within their feature folder, co-located with the feature they belong to. The namespace mirrors the folder path — e.g. `MediaPlatform\Digest\Enums\` or `MediaPlatform\Podcasts\Publishing\PostProduction\Enums\`.
...
- Ownership checks should resolve the model, check ownership, and `return redirect()->route(...)->with('error', '...')` — not `abort_if()`. See `PodcastLinkController::authorizeOwnership()` for the pattern.
...
### Morph Aliases
- Morph aliases are registered in `AppServiceProvider`: `youtube_channel`, `text_based_rss_feed`, `podcast`, `podcast_show`, `digest_list`.
...
- `database/migrations/media_platform/podcasts/` — podcast migrations
...
- Never use `Str::slug()` in production code — always use the custom `makeSlug()` helper (preserves dots). `Str::slug()` is acceptable in test factories only.
...
## Routes
- Each feature has its own dedicated routes file inside its feature folder under a `Routes/` subfolder
...
- Views: `views/` (root level)
- Features: `MEDIA_PLATFORM/FeatureName/Controllers|Models|Services|Requests`
- Tests: `tests/Feature/MEDIA_PLATFORM/FeatureName/`
- Enums: co-located in `Enums/` within their feature folder, e.g. `MEDIA_PLATFORM/Digest/Enums/` or `MEDIA_PLATFORM/Podcasts/Publishing/PostProduction/Enums/`
- Migrations: `database/migrations/media_platform/<feature>/` — registered explicitly in `AppServiceProvider`
